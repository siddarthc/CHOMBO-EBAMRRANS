...
//      EBLevelDataOps::setVal(   *m_pres[ilev], 0.0);
      ...
      m_ibc->initializePressureGradient(*m_gphi[ilev], m_grids[ilev], m_ebisl[ilev], m_domain[ilev], RealVect::Zero, m_time, dxLev); 
      m_ibc->initializePressure(*m_pres[ilev], m_grids[ilev], m_ebisl[ilev], m_domain[ilev], RealVect::Zero, m_time, dxLev);
    }
...
EBAMRNoSubcycle::writeCheckpointFile()
{
  ...
  header.m_int ["max_levels"]             = m_params.m_maxLevel;
  ...
  if (m_extraSolverDefined) writeExtraSolverCheckpoint(handleOut);  
  handleOut.close();
}
/*****************/
/*****************/
void
EBAMRNoSubcycle::
writeExtraSolverCheckpoint(HDF5Handle& a_handle)
{
  for (int ilev = 0; ilev <= m_finestLevel; ilev++)
    {
      m_amrlevels[ilev]->writeCheckpointHeader(a_handle);
      m_amrlevels[ilev]->writeCheckpointLevel(a_handle);     
    }
}
/*****************/
/*****************/
